...
from basic_app.views import IndexView, ChangePasswordView, AdminView, NewUserView, AuthView, DeleteUserView, EditUserView, dummy_function,\
    UploadText, UploadFile, GetItems, DownloadFile, DownloadText, DeleteText, DeleteFile, MakeShareable, CheckIsShareable, DownloadShareable
...
urlpatterns = [
    ...
    path('make_shareable/<int:id>', MakeShareable.as_view(), name='make_shareable'),
    path('is_shareable/<int:id>', CheckIsShareable.as_view(), name='is_shareable'),
    path('get_items/<int:howmany>', GetItems.as_view(), name='get_items'),
    path('download_shareable/<str:url>', DownloadShareable.as_view(), name='download_shareable'),
